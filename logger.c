...
void log_message(FILE *logfile, const char *client_ip, const char *request, int status) {
    ...
    // Mengonversi waktu menjadi format string yang lebih mudah dibaca
    strftime(timestamp, sizeof(timestamp), "%Y-%m-%d %H:%M:%S", localtime(&now));
    
    // Menulis pesan log ke file, dengan format: [timestamp] IP: <client_ip> | Request: <request> | Status: <status>
    fprintf(logfile, "[%s] IP: %s | Request: %s | Status: %d\n", timestamp, client_ip, request, status);
    
    // Menyimpan perubahan pada file log dengan segera
    fflush(logfile);
}
